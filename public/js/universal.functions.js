function validateForm() {
  var requiredFormElements = $('.multi-form .required'),
    phoneInput = $(
      'div[data-index=' + selectedPanel + '].form-panel .bfh-phone.required'
    ),
    checkBoxes = $(
      'div[data-index=' +
        selectedPanel +
        '].form-panel .checkbox-group.required'
    ),
  ...
  for (var i = requiredFormElements.length - 1; i >= 0; i--) {
    if (
      (requiredFormElements[i].value === '' ||
        requiredFormElements[i].value === 'null') &&
      $(requiredFormElements[i]).is(':visible')
    ) {
      $('input[name=' + requiredFormElements[i].name + ']')
        .parent()
        .addClass('has-error');
      $('select[name=' + requiredFormElements[i].name + ']').addClass(
        'has-error'
      );
    }
  }
  ...
  for (var i = requiredFormElements.length; i > 0; i--) {
    if (
      (requiredFormElements[i - 1].value === '' ||
        requiredFormElements[i - 1].value === 'null') &&
      $(requiredFormElements[i - 1]).is(':visible')
    ) {
      return false;
    }
  }
  ...
  for (var i = dependentInputs.length - 1; i >= 0; i--) {
    var controlInput = $(
      '[name=' + $(dependentInputs[i]).attr('dependentOn') + ']:checked'
    );
    if (controlInput.attr('dependentValue') === 'true') {
      // validate
      if (
        dependentInputs[i].value === '' ||
        dependentInputs[i].value === 'null'
      ) {
        $('[name=' + dependentInputs[i].name + ']')
          .parent()
          .addClass('has-error');
        $('[name=' + dependentInputs[i].name + ']').addClass('has-error');
        $(
          '[name=' +
            $('[name=' + dependentInputs[i].name + ']').attr('dependentOn') +
            ']'
        )
          .parent()
          .parent()
...
function crossFadeBG(selectedPanel, panelAdj) {
  ...
  var imgURL = $('div[data-index=' + nextPanel + '] .updateBackground').attr(
    'href'
  );
  ...
  if (imgURL != null) {
    $('#matte').show();
    $('.overlay').fadeOut();
    $('.overlay[img-index=' + nextPanel + ']').css({
